...
// Define the interface for a medical log item
...
const MedicalLogsPage = () => { // Changed to arrow function
  const [medicalLogs, setMedicalLogs] = useState<MedicalLog[] | null>(null); // Initialize with null
    ...
    async function fetchMedicalLogs() {
      ...
      try {
        ...
        if (Array.isArray(data)) {
          ...
          setMedicalLogs([]); // Set empty if data is not array
        }
      } catch (err) {
        console.error(err);
        setError(err instanceof Error ? err.message : 'An unknown error occurred');
        setMedicalLogs([]); // Set empty on error
      }
    }
